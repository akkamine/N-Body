...
int main(int argc, char **argv){
  ...
  MPI_Bcast(&nb_bodies_loc,1,MPI_INT,0,MPI_COMM_WORLD);
	...
	COORDINATES_T* p_pos_x = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_pos_y = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_pos_z = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_sx = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_sy = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_sz = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_fx = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_fy = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_fz = malloc(sizeof(COORDINATES_T)*nb_bodies_loc);	
	COORDINATES_T* p_values = malloc(sizeof(VALUES_T)*nb_bodies_loc);	

	
	MPI_Scatter(bodies.p_pos_x,nb_bodies_loc,MPI_DOUBLE, p_pos_x,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_pos_y,nb_bodies_loc,MPI_DOUBLE, p_pos_y,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_pos_z,nb_bodies_loc,MPI_DOUBLE, p_pos_z,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_sx,nb_bodies_loc,MPI_DOUBLE, p_sx,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_sy,nb_bodies_loc,MPI_DOUBLE, p_sy,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_sz,nb_bodies_loc,MPI_DOUBLE, p_sz,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_fx,nb_bodies_loc,MPI_DOUBLE, p_fx,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_fy,nb_bodies_loc,MPI_DOUBLE, p_fy,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_fz,nb_bodies_loc,MPI_DOUBLE, p_fz,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Scatter(bodies.p_values,nb_bodies_loc,MPI_DOUBLE, p_values,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
	...
	memcpy(posx_tmp, p_pos_x,sizeof(COORDINATES_T)*nb_bodies_loc);
	memcpy(posy_tmp, p_pos_y,sizeof(COORDINATES_T)*nb_bodies_loc);
	memcpy(posz_tmp, p_pos_z,sizeof(COORDINATES_T)*nb_bodies_loc);
	memcpy(values_tmp, p_values,sizeof(VALUES_T)*nb_bodies_loc);
  ...
  /******************************************************************************************/
		...
		while ( FMB_Info.dt*tnow < FMB_Info.tend ) { 
				...
				bodies_Compute_own_interaction_MPI(p_pos_x,p_pos_y,p_pos_z,p_fx,p_fy,p_fz,p_values,nb_bodies_loc,p_pos_x,p_pos_y,p_pos_z,p_values);
				...
				for(i=1;i<nb_proc;i++){
					if(my_rank%2==0){
						...
						//bodies_Compute_own_interaction_MPI(&bodies_loc,posx_tmp,posy_tmp,posz_tmp,values_tmp);
					}
					else{
					...
					bodies_Compute_own_interaction_MPI(p_pos_x,p_pos_y,p_pos_z,p_fx,p_fy,p_fz,p_values,nb_bodies_loc,posx_tmp,posy_tmp,posz_tmp,values_tmp);
					  
				}
				
    		for(i=0;i<nb_bodies_loc;i++){
    			prod = FMB_Info.dt/p_values[i];
    			p_sx[i] += p_fx[i]*prod;
    			p_sy[i] += p_fy[i]*prod;
    			p_sz[i] += p_fz[i]*prod;
    			p_pos_x[i] += p_sx[i]*FMB_Info.dt;
    			p_pos_y[i] += p_sy[i]*FMB_Info.dt;
    			p_pos_z[i] += p_sz[i]*FMB_Info.dt;
				}
		
		if((FMB_Info.save || FMB_Info.sum) && nb_steps%100==0){
			MPI_Gather(p_pos_x,nb_bodies_loc,MPI_DOUBLE,bodies.p_pos_x,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
			MPI_Gather(p_pos_y,nb_bodies_loc,MPI_DOUBLE,bodies.p_pos_y,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
			MPI_Gather(p_pos_z,nb_bodies_loc,MPI_DOUBLE,bodies.p_pos_z,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
			MPI_Gather(p_values,nb_bodies_loc,MPI_DOUBLE,bodies.p_values,nb_bodies_loc,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
/*		          fprintf(force_sum,"Somme des forces à l'étape %lu :\nSur x : %1.15e \nSur y : %1.15e \nSur z : %1.15e\n",nb_steps,sumx_glob,sumy_glob,sumz_glob);*/
			...
			} /* if((FMB_Info.save || FMB_Info.sum) && nb_steps%4==0) */
