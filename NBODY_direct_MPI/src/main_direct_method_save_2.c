...
int main(int argc, char **argv){
  ...
  //COORDINATES_T sumx_loc, sumy_loc, sumz_loc;
	...
	if(my_rank==0){
		...
		printf("res fonction : %d \n",bodies_Nb_bodies(&bodies));
	}
  ...
  MPI_Bcast(&nb_bodies_loc,1,MPI_INT,0,MPI_COMM_WORLD);
	...
	*/
	...
	memcpy(posx_tmp,bodies_loc.p_pos_x,nb_bodies_loc);
	memcpy(posy_tmp,bodies_loc.p_pos_y,nb_bodies_loc);
	memcpy(posz_tmp,bodies_loc.p_pos_z,nb_bodies_loc);
	memcpy(values_tmp,bodies_loc.p_values,nb_bodies_loc);
  ...
  /******************************************************************************************/
		...
		while ( FMB_Info.dt*tnow < FMB_Info.tend ) { 
				...
				/*for(i=0;i<nb_bodies_loc;i++){
					Intermediaire_Integration(prod,&bodies_loc,i);
				...
				for(i=1;i<nb_proc;i++){
					if(my_rank%2==0){
						...
						memcpy(posx_send,posx_tmp,nb_bodies_loc);
						memcpy(posy_send,posy_tmp,nb_bodies_loc);
						memcpy(posz_send,posz_tmp,nb_bodies_loc);
						memcpy(values_send,values_tmp,nb_bodies_loc);
				...
				/*
				for(i=0;i<(bodies_loc.nb_bodies);i++)
    			Intermediaire_Integration(&bodies_loc,i);*/
    		for(i=0;i<(bodies_loc.nb_bodies);i++){
