...
int main(int argc, char **argv){
  ...
  //COORDINATES_T sumx_loc, sumy_loc, sumz_loc;
	...
	/****************************** Envoie du nb_bodies_loc *************************************/
  MPI_Bcast(&nb_bodies_loc,1,MPI_INT,0,MPI_COMM_WORLD);
	
	/***************************** Allocation de la structure locale ***************************/
	...
	for(i=0;i<nb_bodies_loc;i++){
		prod=FMB_Info.dt/(2.*bodies_Get_value(&bodies_loc,i));
  ...
  /******************************************************************************************/
		...
		/*************** Précalcul du carré du softening ***************************************/
		...
		while ( FMB_Info.dt*tnow < FMB_Info.tend ) { 
			...
			/************* Copie des positions locales dans les positions circulantes ************/
			...
			bodies_Compute_own_interaction(&bodies_loc,eps_carre);
			...
			for(i=0;i<nb_proc;i++){
				...
				bodies_Compute_own_interaction_MPI(&bodies_loc,posx_tmp,posy_tmp,posz_tmp,values_tmp,eps_carre);	  
			}
				
			/* Time integration */
			#pragma omp simd
  		for(i=0;i<nb_bodies_loc;i++){
				COORDINATES_T prod2 = (FMB_Info.dt)/bodies_loc.p_values[i];

				bodies_loc.p_pos_x[i] += FMB_Info.dt * bodies_loc.p_sx[i];
				bodies_loc.p_pos_y[i] += FMB_Info.dt * bodies_loc.p_sy[i];
				bodies_loc.p_pos_z[i] += FMB_Info.dt * bodies_loc.p_sz[i];
		
				bodies_loc.p_sx[i] += prod2 * bodies_loc.p_fx[i];
				bodies_loc.p_sy[i] += prod2 * bodies_loc.p_fy[i];
				bodies_loc.p_sz[i] += prod2 * bodies_loc.p_fz[i];
			}
			if(FMB_Info.save ){
...
/*            MPI_Reduce(&sumz_loc,&sumz_glob,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);*/
	...
	/* Frees */
	free(bodies_loc.p_pos_x);
	free(bodies_loc.p_pos_y);
	free(bodies_loc.p_pos_z);
	free(bodies_loc.p_sx);
	free(bodies_loc.p_sy);
	free(bodies_loc.p_sz);
	free(bodies_loc.p_fx);
	free(bodies_loc.p_fy);
	free(bodies_loc.p_fz);
	free(bodies_loc.p_values);
