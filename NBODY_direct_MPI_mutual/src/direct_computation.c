  ...
  for(i=0;i<(p_b_fixe->nb_bodies);i++){
    for(j=0;j<(p_b_fixe->nb_bodies);j++){
      Pxjmi=posx_tmp[j]-bodies_Get_pos_x(p_b_fixe, i);
			...
			bodies_Set_fx(p_b_fixe, i, bodies_Get_fx(p_b_fixe,i)+temp*Pxjmi);
			bodies_Set_fy(p_b_fixe, i, bodies_Get_fy(p_b_fixe,i)+temp*Pyjmi);
			bodies_Set_fz(p_b_fixe, i, bodies_Get_fz(p_b_fixe,i)+temp*Pzjmi);
    }
  }
}

void bodies_Compute_MUTUAL_interaction_MPI(bodies_t *FMB_RESTRICT p_b_fixe, COORDINATES_T* posx_tmp, COORDINATES_T* posy_tmp, COORDINATES_T* posz_tmp, VALUES_T* values_tmp,COORDINATES_T* fx_tmp,COORDINATES_T* fy_tmp,COORDINATES_T* fz_tmp){
