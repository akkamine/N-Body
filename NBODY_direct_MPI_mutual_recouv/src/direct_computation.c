...
void one_body_interaction(bodies_t *FMB_RESTRICT p_b, int num_body, COORDINATES_T eps_carre){
  ...
  for(j=0;j<num_body;j++){
  	Pxjmi=bodies_Get_pos_x(p_b, j)-bodies_Get_pos_x(p_b, num_body);
		Pyjmi=bodies_Get_pos_y(p_b, j)-bodies_Get_pos_y(p_b, num_body);
		Pzjmi=bodies_Get_pos_z(p_b, j)-bodies_Get_pos_z(p_b, num_body);

		r_carre = Pxjmi*Pxjmi + Pyjmi*Pyjmi + Pzjmi*Pzjmi;			
		scal = r_carre + eps_carre;				
		scal *= sqrt(scal);
		mij = bodies_Get_value(p_b, num_body)*bodies_Get_value(p_b, j);
		temp = mij/scal;
		...
		bodies_Set_fx(p_b, num_body, bodies_Get_fx(p_b,num_body)+Fjx);
		bodies_Set_fy(p_b, num_body, bodies_Get_fy(p_b,num_body)+Fjy);
		bodies_Set_fz(p_b, num_body, bodies_Get_fz(p_b,num_body)+Fjz);
			...
			for(j=0;j<i;j++){
		   	...
		   	temp=(bodies_Get_value(p_b, i)*bodies_Get_value(p_b, j))/(scal*sqrt(scal));
				...
				if(rang!=0){
					...
					bodies_Set_fx(p_b, i, bodies_Get_fx(p_b,i)+temp*Pxjmi);
					bodies_Set_fy(p_b, i, bodies_Get_fy(p_b,i)+temp*Pyjmi);
					bodies_Set_fz(p_b, i, bodies_Get_fz(p_b,i)+temp*Pyjmi); 
					bodies_Set_fx(p_b, j, bodies_Get_fx(p_b,j)-temp*Pxjmi);
					bodies_Set_fy(p_b, j, bodies_Get_fy(p_b,j)-temp*Pyjmi);
					bodies_Set_fz(p_b, j, bodies_Get_fz(p_b,j)-temp*Pyjmi); 
		  }
		}
  }
