...
int main(int argc, char **argv){
  ...
  /********************************* Reading files by processus 0 *******************************/
	...
	for(i=0;i<bodies.nb_bodies;i++){
		COORDINATES_T prod = FMB_Info.dt/(2.*bodies.p_values[i]);
		bodies.p_sx[i] += bodies.p_fx[i] * prod;
		bodies.p_sy[i] += bodies.p_fy[i] * prod;
		bodies.p_sz[i] += bodies.p_fz[i] * prod;
	}
  ...
  /*****************************************************************************************/
		...
		while ( FMB_Info.dt*tnow < FMB_Info.tend ) { 
			...
			for(i=0;i<bodies.nb_bodies;i++){
				...
				for(j=0;j<bodies.nb_bodies;j++){			
				  	COORDINATES_T Pxjmi = bodies.p_pos_x[j] - bodies.p_pos_x[i];
				  	COORDINATES_T Pyjmi = bodies.p_pos_y[j] - bodies.p_pos_y[i];
				  	COORDINATES_T Pzjmi = bodies.p_pos_z[j] - bodies.p_pos_z[i];

					COORDINATES_T r_carre = Pxjmi*Pxjmi + Pyjmi*Pyjmi + Pzjmi*Pzjmi;			
					COORDINATES_T scal = r_carre + eps_carre;				
					scal *= sqrt(scal);
					COORDINATES_T mij = bodies.p_values[i] * bodies.p_values[j];
			...
			#pragma omp simd
	  		for(i=0;i<bodies.nb_bodies;i++){
				COORDINATES_T prod2 = (FMB_Info.dt)/bodies.p_values[i];

				bodies.p_pos_x[i] += FMB_Info.dt * bodies.p_sx[i];
				bodies.p_pos_y[i] += FMB_Info.dt * bodies.p_sy[i];
				bodies.p_pos_z[i] += FMB_Info.dt * bodies.p_sz[i];
			
				bodies.p_sx[i] += prod2 * bodies.p_fx[i];
				bodies.p_sy[i] += prod2 * bodies.p_fy[i];
				bodies.p_sz[i] += prod2 * bodies.p_fz[i];
